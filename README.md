...
Development-usable assets and structure
Editable Figma source reconstruction
Pixel-level visual closeness
...
ai2psd-struct generates the structured image package.
ai2figma-struct consumes that package to rebuild editable Figma layers.
Output Structure
...
Add a package.json.
Add an installer script, for example bin/install.js.
Include SKILL.md, references/, scripts/, and agents/ in the published package.
Test the package locally with npm pack.
Install the packed .tgz file globally and confirm the skill appears under ~/.codex/skills/ai2psd-struct.
Publish to npm with npm publish --access public.
Add the npm install command to this README.
